...
contract Uniswap2AdapterV4 is ERC2771Context {
     ...
     */
    constructor(address _marketplace, address _projectRegistry, address _uniswapRouter) ERC2771Context(address(0x0)) {
    ...
    function buyWithERC20(bytes32 productId, uint minSubscriptionSeconds, uint timeWindow, address erc20Address, uint amount) public {
        ...
        if (pricePerSecond == 0x0) {
            ...
            marketplace.buyFor(productId, minSubscriptionSeconds, subscriber);
            return;
        }
        ...
        _buyWithUniswap(subscriber, productId, minSubscriptionSeconds, timeWindow, pricePerSecond, amount, erc20Address, pricingTokenAddress);
    }
    ...
    function buyWithETH(bytes32 productId, uint minSubscriptionSeconds, uint timeWindow) public payable {
        ...
        _buyWithUniswap(subscriber, productId, minSubscriptionSeconds, timeWindow, pricePerSecond, msg.value, uniswapRouter.WETH(), pricingTokenAddress);
    }
     ...
     */
    function _buyWithUniswap(address subscriber, bytes32 productId, uint minSubscriptionSeconds, uint timeWindow, uint pricePerSecond, uint amount, address fromToken, address toToken) internal{
        ...
        marketplace.buyFor(productId, subscriptionSeconds, subscriber);
    }
    ...
    function _getPriceInfo(bytes32 productId) internal view returns (uint, address) {
        ...
        require(projectRegistry.exists(productId), "error_productDoesNotExist");
