...
const { log } = console;
const { KEY = "0x5e98cce00cff5dea6b454889f359a4ec06b9fa6b88e9d69b86de8e1c81887da0", DELAY = "0", GAS_PRICE_BUMP_PERCENT, 
// Easy setting: read addresses and URLs from @streamr/config
ENS_CHAIN = "dev2", REGISTRY_CHAIN = "dev2", 
// Individual overrides
ENS_RPC_URL, REGISTRY_RPC_URL, ENS_ADDRESS, REGISTRY_ADDRESS, ENS_CACHE_ADDRESS, HEARTBEAT_FILENAME, } = process.env;
const gasPriceBumpPercent = parseInt(GAS_PRICE_BUMP_PERCENT ?? "0");
if (isNaN(gasPriceBumpPercent)) {
    throw new Error(`GAS_PRICE_BUMP_PERCENT="${GAS_PRICE_BUMP_PERCENT}" is not a valid number! Try e.g. 20`);
}
if (gasPriceBumpPercent > 0) {
    log(`Will bump gas price by ${gasPriceBumpPercent}%`);
}
const delay = (parseInt(DELAY) || 0) * 1000;
if (delay > 0) {
    log(`Starting with answer delay ${delay} milliseconds`);
}
const ensChainRpc = ENS_RPC_URL ?? config[ENS_CHAIN]?.rpcEndpoints?.[0]?.url;
if (!ensChainRpc) {
    throw new Error(`Either ENS_CHAIN or ENS_RPC_URL must be set in environment`);
}
const ensChainProvider = new JsonRpcProvider(ensChainRpc);
const registryChainRpc = REGISTRY_RPC_URL ?? config[REGISTRY_CHAIN]?.rpcEndpoints?.[0]?.url;
if (!registryChainRpc) {
    throw new Error(`Either REGISTRY_CHAIN or REGISTRY_RPC_URL must be set in environment`);
}
...
const ensAddress = ENS_ADDRESS ?? config[ENS_CHAIN]?.contracts?.ENS;
if (!ensAddress) {
    throw new Error(`Either ENS_CHAIN or ENS_ADDRESS must be set in environment`);
}
const ensContract = new Contract(ensAddress, ensRegistryABI, ensChainProvider);
const registryAddress = REGISTRY_ADDRESS ?? config[REGISTRY_CHAIN]?.contracts?.StreamRegistry;
if (!registryAddress) {
    throw new Error(`Either REGISTRY_CHAIN or REGISTRY_ADDRESS must be set in environment`);
}
const streamRegistryContract = new Contract(registryAddress, streamRegistryABI, registryChainProvider);
const ensCacheAddress = ENS_CACHE_ADDRESS ?? config[REGISTRY_CHAIN]?.contracts?.ENSCacheV2;
if (!ensCacheAddress) {
    throw new Error(`Either REGISTRY_CHAIN or ENS_CACHE_ADDRESS must be set in environment`);
}
const ensCacheContract = new Contract(ensCacheAddress, ENSCacheV2ABI, registryChainWallet);
...
async function handleEvent(ensName, streamIdPath, metadataJsonString, requestorAddress) {
...
async function createStream(ensName, streamIdPath, metadataJsonString, requestorAddress, retry = false) {
