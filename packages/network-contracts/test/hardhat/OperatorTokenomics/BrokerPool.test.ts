...
describe("BrokerPool", (): void => {
    ...
    it("stakes, and unstakes with gains", async function(): Promise<void> {
        ...
        await expect(pool.unstake(sponsorship.address))
            .to.emit(pool, "Unstaked").withArgs(sponsorship.address)
    ...
    it("updates approximate pool value when updateApproximatePoolvalueOfSponsorship is called", async function(): Promise<void> {
        const { token } = sharedContracts
        await setTokens(sponsor, "1000")
        ...
        await advanceToTimestamp(timeAtStart, "Stake to sponsorship")
        await (await pool.stake(sponsorship.address, parseEther("1000"))).wait()
        ...
        const poolValuePerSponsorshipBefore = await pool.getApproximatePoolValuesPerSponsorship()

        await (await pool.updateApproximatePoolvalueOfSponsorship(sponsorship.address)).wait()
        ...
        const poolValuePerSponsorshipAfter = await pool.getApproximatePoolValuesPerSponsorship()
    ...
    it("tries to keep the maintenance margin, diverts all of broker's share", async function(): Promise<void> {
        ...
        await pool.withdrawEarningsFromSponsorship(sponsorship.address)
    ...
    it("keeps the maintenance margin, diverts enough of broker's share to reach maintenanceMarginPercent", async function(): Promise<void> {
        ...
        await setTokens(delegator, "1000")
        const sponsorship = await deploySponsorship(sharedContracts)
        await (await dataToken.connect(sponsor).transferAndCall(sponsorship.address, parseEther("1000"), "0x")).wait()
        ...
        await expect(pool.stake(sponsorship.address, parseEther("400")))
            .to.emit(pool, "Staked").withArgs(sponsorship.address)
            .to.emit(pool, "StakeUpdate").withArgs(sponsorship.address, parseEther("400"))
    ...
    it("revenue sharing scenarios 1..7: happy path pool life cycle", async function(): Promise<void> {
        ...
        // 2: Simple Staking
        await advanceToTimestamp(timeAtStart, "Stake to sponsorship")
        await expect(pool.stake(sponsorship.address, parseEther("500")))
            .to.emit(pool, "Staked").withArgs(sponsorship.address)
            .to.emit(pool, "StakeUpdate").withArgs(sponsorship.address, parseEther("500"))
        ...
        expect(await pool.getPoolValueFromSponsorship(sponsorship.address)).to.equal(parseEther("500"))
        expect(await sponsorship.stakedWei(pool.address)).to.equal(parseEther("500"))
        expect(await sponsorship.getEarnings(pool.address)).to.equal(parseEther("0"))
        ...
        await advanceToTimestamp(timeAtStart + 10000, "Withdraw from sponsorship") // sponsorship only has 25 DATA sponsorship, so that's what it will allocate
        await (await pool.withdrawEarningsFromSponsorship(sponsorship.address)).wait()
        // TODO: add event to BrokerPool
        // await expect(pool.withdrawEarningsFromSponsorship(sponsorship.address))
        //    .to.emit(pool, "Withdrawn").withArgs(sponsorship.address, parseEther("2500"))
    ...
    it.skip("revenue sharing scenarios 8..10: slashing", async function(): Promise<void> {
        ...
        // - There is one pool that has staked 5 DATA into the sponsorship
        ...
        await expect(pool.stake(sponsorship.address, parseEther("5")))

        // 8: Slashing
        await expect(sponsorship.voteOnFlag(pool.address, parseEther("5").toHexString()))
            .to.emit(sponsorship, "StakeUpdate").withArgs(pool.address, parseEther("0"), parseEther("0"))
    ...
    describe("Undelegation queue", function(): void {
        ...
        it("pays out 1 queue entry fully using earnings withdrawn from sponsorship", async function(): Promise<void> {
            ...
            await advanceToTimestamp(timeAtStart, "Stake to sponsorship + queue the payout") // no free funds in the pool => no payout
            await expect(pool.stake(sponsorship.address, parseEther("1000")))
                .to.emit(pool, "Staked").withArgs(sponsorship.address)
            ...
            await expect(pool.withdrawEarningsFromSponsorship(sponsorship.address))
            // TODO: add event to BrokerPool
            //    .to.emit(pool, "EarningsWithdrawn").withArgs(sponsorship.address, parseEther("1000"))
                .to.emit(pool, "Undelegated").withArgs(delegator.address, parseEther("180"))
            //    .to.emit(pool, "BrokerSharePaid").withArgs(sponsorship.address, parseEther("200"))
        ...
        it("accepts forced takeout from non-broker after grace period is over (negative + positive test)", async function(): Promise<void> {
            ...
            await expect (pool.connect(delegator).forceUnstake(sponsorship.address, 10)).to.be.revertedWith("error_onlyBroker")
            ...
            await (await pool.connect(delegator).forceUnstake(sponsorship.address, 10)).wait()
    ...
    it("forced example scenario", async function(): Promise<void> {
        ...
        await pool.stake(sponsorship1.address, parseEther("200"))
        await pool.stake(sponsorship2.address, parseEther("100"))
        ...
        await expect(pool.connect(delegator).forceUnstake(sponsorship1.address, 100)).to.be.revertedWith("error_onlyBroker")

        await advanceToTimestamp(timeAtStart + 31*days, "Broker unstakes 5 data from sponsorship1")
        await pool.reduceStakeTo(sponsorship1.address, parseEther("150"))
        ...
        // now anyone can trigger the unstake and payout of the queue
        // await (await pool.updateApproximatePoolvalueOfSponsorship(sponsorship2.address)).wait()
        // await (await pool.updateApproximatePoolvalueOfSponsorship(sponsorship1.address)).wait()
        await expect(pool.connect(delegator2).forceUnstake(sponsorship1.address, 10))
            .to.emit(pool, "Unstaked").withArgs(sponsorship1.address)
    ...
    it("edge case many queue entries, one sponsorship", async function(): Promise<void> {
        ...
        await pool.unstake(sponsorship.address, { gasLimit: 0xF42400 })
    ...
    it("punishes broker on too much diff on approx poolvalue", async function(): Promise<void> {
        ...
        const timeAtStart = await getBlockTimestamp()
        await advanceToTimestamp(timeAtStart, "Stake to sponsorship")
        await expect(pool.stake(sponsorship1.address, parseEther("500")))
            .to.emit(pool, "Staked").withArgs(sponsorship1.address)
        await expect(pool.stake(sponsorship2.address, parseEther("500")))
            .to.emit(pool, "Staked").withArgs(sponsorship2.address)
        ...
        await pool.connect(delegator).updateApproximatePoolvalueOfSponsorships([sponsorship1.address, sponsorship2.address])
    ...
    it("gets notified when kicked (IBroker interface)", async function(): Promise<void> {
        ...
        await pool.updateApproximatePoolvalueOfSponsorships([sponsorship.address])
        expect(await pool.getApproximatePoolValue()).to.equal(parseEther("2000"))

        // TestKickPolicy actually kicks and slashes given amount
        await expect(sponsorship.connect(admin).voteOnFlag(pool.address, hexZeroPad(parseEther("10").toHexString(), 32)))
            .to.emit(sponsorship, "BrokerKicked").withArgs(pool.address)
            .to.emit(sponsorship, "BrokerSlashed").withArgs(pool.address, parseEther("10"))
    ...
    it("reduces pool value when it gets slashed without kicking (IBroker interface)", async function(): Promise<void> {
        ...
        await (await sponsorship.connect(admin).flag(pool.address)).wait() // TestKickPolicy actually slashes 10 ether without kicking
        expect(await pool.getApproximatePoolValue()).to.equal(parseEther("1990"))
    })
    ...
    it("will NOT let anyone else to stake except the broker of the BrokerPool", async function(): Promise<void> {
        ...
        await expect(pool.connect(admin).stake(sponsorship.address, parseEther("1000")))
    ...
    it("will NOT allow staking to non-Sponsorships", async function(): Promise<void> {
        const pool = await deployBrokerPool(sharedContracts, broker)
        await (await sharedContracts.token.mint(pool.address, parseEther("1000"))).wait()
        await expect(pool.stake(sharedContracts.token.address, parseEther("1000"))).to.be.revertedWith("error_badSponsorship")
    })
    ...
    it("will NOT allow staking to Sponsorships that were not created using the correct SponsorshipFactory", async function(): Promise<void> {
        ...
        await (await sharedContracts.token.mint(pool.address, parseEther("1000"))).wait()
        await expect(pool.stake(badSponsorship.address, parseEther("1000")))
