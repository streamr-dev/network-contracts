...
describe("MaxAmountBrokersJoinPolicy", (): void => {
    ...
    it("will NOT let too many brokers join", async function(): Promise<void> {
        const { token } = contracts
        const sponsorship = await deploySponsorshipWithoutFactory(contracts, { maxBrokerCount: 1 })
        await expect(token.transferAndCall(sponsorship.address, parseEther("1"), broker.address))
            .to.emit(sponsorship, "BrokerJoined")
        await expect(token.transferAndCall(sponsorship.address, parseEther("1"), broker2.address))
