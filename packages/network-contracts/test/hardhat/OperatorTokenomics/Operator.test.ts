...
describe("Operator contract", (): void => {
    ...
    describe("Delegation management", (): void => {
        ...
        it("keeps track of delegators queuing to undelegate", async function(): Promise<void> {
            ...
            expect((await operator.undelegationQueue()).map((q) => q.delegator)).to.deep.equal([ delegator.address, delegator2.address ])
            ...
            await (await operator.connect(delegator).undelegate(parseEther("300"))).wait()
            ...
            expect((await operator.undelegationQueue()).map((q) => q.delegator)).to.deep.equal([
                delegator.address, delegator2.address, delegator.address, delegator2.address
            ])
            ...
            expect((await operator.undelegationQueue()).map((q) => q.delegator)).to.deep.equal([
                delegator.address, delegator2.address, delegator3.address
            ])
            expect((await operator.undelegationQueue()).map((q) => q.amountWei)).to.deep.equal([
