...
describe('ERC1155JoinPolicy', (): void => {
    ...
    it('should fail to complete withdrawStake, reason: stakingDisabled', async () => {
        await expect(contract.connect(wallets[0]).withdrawStake(TokenIds.A, 0)).to.be.revertedWith(
            'stakingDisabled'
        )
    })
    ...
    it('should fail to grant permissions if account is not authorized on DelegatedAccessRegistry', async () => {
        await expect(
            contract.connect(wallets[1]).requestDelegatedJoin(TokenIds.A)
    ...
    it('should grant 1 token to a user and fullfil their requestDelegatedJoin', async () => {
        ...
        await contract.connect(wallets[0]).requestDelegatedJoin(TokenIds.A)

        const events = await contract.queryFilter(contract.filters.Accepted())
        expect(events.length).to.equal(1)
        expect(events[0].args).to.not.be.undefined

        expect(events[0].args!.mainWallet).to.equal(wallets[0].address)
    ...
    it('should allow for a main account to be granted access via requestJoin', async () => {
        await token
            .connect(wallets[0])
            .safeTransferFrom(wallets[0].address, wallets[5].address, TokenIds.A, 1, '0x')
    ...
    describe('ERC1155JoinPolicy - StakeGate', async () => {
        ...
        before(async (): Promise<void> => {
            const ERC1155JoinPolicy = await ethers.getContractFactory(
                'ERC1155JoinPolicy',
                wallets[0]
            )
        ...
        it('should exercise the depositStake method, happy-path', async () => {
            ...
            await stakedContract.connect(mainWallet).depositStake(TokenIds.C, TokenAmount)
        ...
        it('should exercise the withdrawStake, happy-path', async () => {
            ...
            await stakedContract.connect(mainWallet).withdrawStake(TokenIds.C, contractBalance)
        ...
        it('should fail to call withdrawStake, reason: insufficient balance', async () => {
            await expect(
                stakedContract.connect(mainWallet).withdrawStake(TokenIds.C, TokenAmount)
