...
describe('ERC20JoinPolicy', (): void => {
    ...
    it('should fail to leave a stream if not joined', async () => {
        await expect(contract.requestDelegatedLeave()).to.be.revertedWith('error_notAuthorized')
    })
    ...
    it('should revoke permissions for delegated wallet, if joined', async () => {
        await contract.connect(wallets[1]).requestDelegatedLeave()
