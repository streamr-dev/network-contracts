...
export class Migrator {
    ...
    constructor() {
        ...
        const mtx = await this.registryFromAdmin.grantRole(await this.registryFromAdmin.TRUSTED_ROLE(), migratorWallet.address)
        await mtx.wait()
...
const sendStreamsToChain = async (streams: StreamData[], nonceParam: number) => {
    ...
    try {
        ...
        if (err.code === 'TRANSACTION_REPLACED') { console.log('a transaction got replaced') } else { console.log(err) }
    }
}
...
const addAndSendStreamPermission = async (streamID: string, user:string, permissionStrings:string[], lineNr: number) => {
    ...
    streamsToMigrate.push({
        id: streamID, user, permissions
    })
    if (streamsToMigrate.length >= 60) {
        const clonedArr = streamsToMigrate.map((a) => ({
            ...a
        }))
