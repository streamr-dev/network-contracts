...
async function deploySponsorshipFactory() {
    ...
    const hasroleEthSigner = await streamrConfig.hasRole(await streamrConfig.ADMIN_ROLE(), adminWallet.address)
    log(`hasrole(ADMIN_ROLE) adminwallet ${hasroleEthSigner}`)
    ...
    const hasrole = await sponsorship.hasRole(await sponsorship.ADMIN_ROLE(), adminWallet.address)
