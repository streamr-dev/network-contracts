import { StreamCreated, TransferedPublishRights, TransferedViewRights } from '../generated/StreamRegistry/StreamRegistry'
...
export function handleStreamCreation(event: StreamCreated): void {
  let stream = new Stream(event.params.id.toHex())
  stream.metadata = event.params.metadata
  stream.save()
  let permissionId = event.params.id.toHex() + '-' + event.params.owner.toHex()
  let permission = createPermission(permissionId, 
...
export function handleTransferViewRights(event: TransferedViewRights): void {
  ...
  let permissionSenderId = event.params.streamid.toHex() + '-' + event.params.from.toHex()
  let permissionRecipientId = event.params.streamid.toHex() + '-' + event.params.to.toHex()
  let permissionSender = Permission.load(permissionSenderId)
